module github.com/evergreen-ci/logkeeper

go 1.16

require (
	github.com/evergreen-ci/pail v0.0.0-20221122205930-80cb99a15093
	github.com/evergreen-ci/render v0.0.0-20141211045555-c9e0e54c798f
	github.com/evergreen-ci/utility v0.0.0-20220725171106-4730479c6118
	github.com/gorilla/handlers v1.5.1
	github.com/gorilla/mux v1.8.0
	github.com/mongodb/amboy v0.0.0-20220705160922-807d9e896cb4
	github.com/mongodb/grip v0.0.0-20220401165023-6a1d9bb90c21
	github.com/pkg/errors v0.9.1
	github.com/stretchr/testify v1.8.1
	github.com/urfave/negroni v1.0.0
	gonum.org/v1/gonum v0.11.0
	gopkg.in/mgo.v2 v2.0.0-20190816093944-a6b53ec6cb22
)
